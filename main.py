            ...
            if response.status_code == 200:
                ...
                if download_response.status_code == 200:
                    upload_response = requests.put(
                        upload_url, data=download_response.content
                    )
                    if upload_response.status_code == 201:
                        return f"Файл '{name_file}' успешно загружен."
                    else:
                        return (
                            f"Ошибка при загрузке файла: {upload_response.status_code}"
                        )
                else:
                    return (
                        f"Ошибка при скачивании файла: {download_response.status_code}"
                    )
            else:
                return (
                    f"Ошибка при получении ссылки на загрузку: {response.status_code}"
                )
...
def get_top_photos(photos_vk_all, top_n=5):
    ...
    photos_top_sorted = sorted(
        photos_vk.items(), key=lambda item: item[1][0], reverse=True
    )

    photo_top = {}  # Отсортированный словарь
    for number_photo, (url, (resolution, likes)) in enumerate(
        photos_top_sorted[:top_n], start=1
    ):
...
def top_res_photo_vk_upload_in_yd():
    ...
    name_folder_yd = input(
        "Введите название папки на яндекс диске, в которую загрузить файлы: "
    )
    try:
        top_n = int(
            input("Введите число фотографий с наилучшим разрешением (по умолчанию 5): ")
            or "5"
        )
    except ValueError:
    ...
    with tqdm(total=len(photos_top), desc="Загрузка файлов", unit="file") as pbar:
        for url, (name_file, _) in photos_top.items():
            upload_status = yd_api.upload_files(
                name_file=name_file, url_download=url, folder_name=name_folder_yd
            )
